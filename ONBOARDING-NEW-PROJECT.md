...
> 각 화면 L1 specs 파일을 만든 후, 출력 항목·입력 필드가 있는 화면마다 실행합니다.
> 예: FZ009(접수·현황 출력항목), FZ016(연차정보 입력 필수값) 등

**준비 (1회)**:
DB 명세서 파일을 tc-pilot/ 폴더에 복사해 두세요.
Claude Code가 같은 폴더에 있는 파일을 자동으로 읽습니다.

**연차료 관련 테이블 목록 먼저 파악하기:**

```
IPAZON_DB명세서_V3_9_2.xlsx 에서
"연차" 또는 "annuity" 또는 "fee" 가 포함된 테이블명을 전부 추출해서
목록으로 보여줘.
```

**화면별 L5 JSON 생성 프롬프트 (TC 생성 직전에 실행):**

```
IPAZON_DB명세서_V3_9_2.xlsx 와 specs/FZ009.md 를 읽고
data/FZ009-fields.json 을 생성해줘.

FZ009는 접수·현황 출력 항목 화면이야.
아래 조건으로 필드를 추출해줘:

1. FZ009 화면에 출력되는 필드만 추출
   (DB 전체 테이블 중 연차료 관련 테이블 집중)
2. 각 필드마다 아래 정보 포함:
   - iz_name: 화면에 표시되는 한글 필드명
   - db_table: 관련 DB 테이블명
   - db_column: DB 컬럼명
   - direction: send(IZ→연차료) / receive(연차료→IZ) / both(양방향)
   - format: 출력 형식 (날짜/숫자/텍스트 등)
   - tc_note: TC 작성 시 주의사항

3. direction:both 필드는 tc_note에 "데이터 정합성 TC 필수" 표기
4. 불확실한 필드는 tc_note에 "[확인필요]" 표기
```

**화면코드만 바꿔서 재사용 가능 — 자주 쓰는 화면:**

| 화면코드 | 화면명 | 프롬프트 포인트 |
|---------|--------|--------------|
| FZ009 | 접수·현황 출력항목 | 연차료 대상건 목록 출력 필드 |
| FZ016 | 연차정보 입력 필수값 | 필수/선택 구분 + 입력방식(텍스트/셀렉트/캘린더) |
| FZ032 | 리마인더 출력항목 | 발송번호·관리번호 연관관계 포함 |
| FZ041 | 납부현황 출력항목 | IZ↔연차료 양방향 데이터 정합성 |
